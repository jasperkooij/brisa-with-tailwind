import{createRequire as N}from"node:module";var C=Object.create;var{getPrototypeOf:F,defineProperty:W,getOwnPropertyNames:E}=Object;var D=Object.prototype.hasOwnProperty;var ke=(e,i,n)=>{n=e!=null?C(F(e)):{};const r=i||!e||!e.__esModule?W(n,"default",{value:e,enumerable:!0}):n;for(let s of E(e))if(!D.call(r,s))W(r,s,{get:()=>e[s],enumerable:!0});return r};var We=(e,i)=>()=>(i||e((i={exports:{}}).exports,i),i.exports);var K=N(import.meta.url);import k from"node:child_process";import we from"node:fs";import qe from"node:path";import c from"node:path";var j="0.1.1";import de from"node:path";import p from"node:path";import{fileURLToPath as h}from"node:url";import{createRequire as I}from"node:module";var ee=typeof Bun!=="undefined";function a(e,i){try{return I(i??p.resolve(process.cwd(),"src")).resolve(e,{paths:i?[p.resolve(i.startsWith("file://")?h(i):i,"..")]:void 0})}catch(n){if(!ee)throw n;return Bun.resolveSync(e,i?p.dirname(i):import.meta.dirname)}}import ie from"node:path";import{pathToFileURL as ne}from"node:url";function se(e,i=re){return i?ne(e).href:e}var re=typeof Bun==="undefined";function m(e,i,n){try{const r=a(ie.join(i,e));return se(r,n)}catch(r){return null}}async function o(e,i=de.join(process.cwd(),"build")){const n=m(e,i);if(!n)return null;return await import(n)}function M(e="",i=0){if(typeof Bun!=="undefined")return Bun.hash(e,i);return ce(e,i)}function ce(e="",i=0){const{ptr:n,size:r}=typeof e==="string"?be(e,!1):H(e);return BigInt.asUintN(64,g.wyhash(n,r,BigInt(i)))}function H(e,i=!1){const n=e.byteLength+ +i;if(n===0)return{ptr:-1,size:0};const r=g.alloc(n);if(r===-1)throw new Error("WASM memory allocation failed");const s=new Uint8Array(g.memory.buffer);if(s.set(new Uint8Array(ArrayBuffer.isView(e)?e.buffer:e),r),i)s[r+e.byteLength]=0;return{ptr:r,size:n}}function be(e,i=!0){const n=me.encode(e);return H(n,i)}var te="",oe=Buffer.from(te,"base64"),{instance:ue}=await WebAssembly.instantiate(oe,{env:{print(e){console.log(e)}}}),g=ue.exports,me=new TextEncoder;import f from"node:process";import xe from"node:os";import X from"node:tty";function le(){const e=f?.env??{};if(!e.FORCE_COLOR)return;if(e.FORCE_COLOR==="true"||e.FORCE_COLOR.length===0)return!0;if(e.FORCE_COLOR==="false")return!1;return Math.min(Number.parseInt(e.FORCE_COLOR,10),3)>0}function J(){const e=f?.env??{},i=le();if(i!==void 0)return i;if(!X.isatty(1)&&!X.isatty(2))return!1;if(f.platform==="win32"){const n=xe.release().split(".");return Number(n[0])>=10&&Number(n[2])>=10586}return e.TERM!=="dumb"}var t=J();var y=(e)=>t?`\x1B[32m${e}\x1B[0m`:e,Q=(e)=>t?`\x1B[33m${e}\x1B[0m`:e,Y=(e)=>t?`\x1B[31m${e}\x1B[0m`:e,Z=(e)=>t?`\x1B[34m${e}\x1B[0m`:e,P=(e)=>t?`\x1B[36m${e}\x1B[0m`:e,v=(e)=>t?`\x1B[1m${e}\x1B[0m`:e;async function S(){const e=["lightningcss"],i=(await o("css-files",u))?.default??[],r=(await o("_integrations",c.resolve(u,"web-components")))?.webContextPlugins??[],s=(await o("i18n",ge))?.default,d={...ve,...(await o("brisa.config",l))?.default??{}},G=ae.has(d?.output);if(s?.pages)s.pages=JSON.parse(JSON.stringify(s.pages,($e,b)=>typeof b==="string"&&b.length>1?b.replace(/\/$/g,""):b));const R=new Set(s?.locales||[]);if(d.basePath&&!d.basePath.startsWith(c.sep))d.basePath=c.sep+d.basePath;if(!d.external)d.external=e;else d.external=[...d.external,...e];return globalThis.__BASE_PATH__=d.basePath,{CSS_FILES:i,CONFIG:d,I18N_CONFIG:s,WEB_CONTEXT_PLUGINS:r,LOCALES_SET:R,IS_STATIC_EXPORT:G}}async function he(){x=globalThis.mockConstants={...x,...await S()}}var{NODE_ENV:B}=process.env,w=Boolean(process.env.IS_PROD)||B==="production"||process.argv.some((e)=>e==="PROD"),q=c.join("brisa","out","cli"),je=Boolean(process.env.IS_SERVE_PROCESS)||Boolean(process.argv[1]?.endsWith?.(c.join(q,"serve","index.js"))),$=Boolean(process.env.IS_STANDALONE_SERVER),V=Boolean(process.argv[1]?.endsWith?.(c.join(q,"build.js"))),pe=process.argv[1]?.replace(new RegExp(`${q}.*`),"brisa"),l=$?import.meta.dirname:process.cwd(),ae=new Set(["static","desktop","android","ios"]),U=$?import.meta.dirname:c.resolve(l,"src"),u=$?import.meta.dirname:process.env.BRISA_BUILD_FOLDER??c.resolve(l,"build"),ge=V?U:u,z="/_404",A="/_500",fe=process.platform!=="darwin"&&process.platform!=="win32",ye=w?"public, max-age=31536000, immutable":"no-store, must-revalidate",ve={trailingSlash:!1,assetPrefix:"",basePath:"",extendPlugins:(e)=>e,output:"bun",clustering:w&&fe,integrations:[]},x={JS_RUNTIME:typeof Bun!=="undefined"?"bun":"node",PAGE_404:z,PAGE_500:A,VERSION:j,VERSION_HASH:M(j),RESERVED_PAGES:[z,A],IS_PRODUCTION:w,IS_DEVELOPMENT:process.argv.some((e)=>e==="DEV")||B==="development",IS_SERVE_PROCESS:je,IS_BUILD_PROCESS:V,PORT:Number.parseInt(process.argv[2])||3000,BUILD_DIR:u,ROOT_DIR:l,BRISA_DIR:pe,SRC_DIR:U,ASSETS_DIR:c.resolve(u,"public"),PAGES_DIR:c.resolve(u,"pages"),LOG_PREFIX:{WAIT:P("[ wait ]")+" ",READY:y("[ ready ] ")+" ",INFO:Z("[ info ] ")+" ",ERROR:Y("[ error ] ")+" ",WARN:Q("[ warn ] ")+" ",TICK:y("\u2713 ")+" "},REGEX:{CATCH_ALL:/\[\[\.{3}.*?\]\]/g,DYNAMIC:/\[.*?\]/g,REST_DYNAMIC:/\[\.{3}.*?\]/g},HEADERS:{CACHE_CONTROL:ye},...await S()},T=()=>globalThis.mockConstants?globalThis.mockConstants:x,ei=x;var _=`import type { Configuration } from "brisa";
import PandaCSS from 'brisa-pandacss';

export default {
  integrations: [PandaCSS()],
} as Configuration;

`;async function L(){const{ROOT_DIR:e,LOG_PREFIX:i}=T(),n=m("brisa.config",e),r=Object.entries({"brisa-pandacss":"0.1.1","@pandacss/dev":"0.46.1",postcss:"8.4.47"});if(console.log(i.WAIT,"Adding PandaCSS dependencies..."),k.spawnSync("bun",["add",...r.map(([s,d])=>`${s}@${d}`)],{stdio:"inherit"}),console.log(i.INFO,i.TICK,"PandaCSS dependencies added!"),k.spawnSync("bun",["panda","init","-p"]),k.spawnSync("bun",["pkg","set",'scripts.prepare="panda codegen"']),console.log(i.INFO,i.TICK,"PandaCSS initialized!"),!n)we.writeFileSync(qe.join(e,"brisa.config.ts"),_),console.log(i.INFO,i.TICK,"PandaCSS configuration added!");else console.log(i.WARN,`Almost there! We detected an existing ${v("brisa.config.ts")} file.`),console.log(i.WARN,"Please add the following configuration to integrate PandaCSS:"),console.log(i.WARN),_.split("\n").forEach((s)=>console.log(i.WARN,v(s)))}if(K.main==K.module)await L(),process.exit(0);export{L as default};
